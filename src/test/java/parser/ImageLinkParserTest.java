...
public class ImageLinkParserTest {
    ...
    @Test
    void getNewsGifLink() {
        final var doc = FileUtils.parseHtmlFile("news_with_gif.html");
        final var element = doc.getElementsByClass("news-element");
        final var link = imageLinkParser.getImageLink(element.first());
        Assertions.assertEquals("https://static.goodgame.ru/files/logotypes/ma_crop_42069_Pltb.gif", link);
    }

    @Test
    void getNewsImageLink() {
        final var doc = FileUtils.parseHtmlFile("news_with_image.html");
        final var element = doc.getElementsByClass("news-element");
        final var link = imageLinkParser.getImageLink(element.first());
    ...
    void getTournamentImageLink() {
        final var doc = FileUtils.parseHtmlFile("tournament_with_image.html");
        final var element = doc.getElementsByClass("news-element");
        final var link = imageLinkParser.getImageLink(element.first());
